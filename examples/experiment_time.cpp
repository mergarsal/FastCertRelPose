...
int main(int argc, char** argv)
{
  ...
  Eigen::MatrixXd array_noise(4, 1);
  Eigen::MatrixXd array_n_points(20, 1);

  array_noise << 0.1, 0.5, 1.0, 2.5;
  array_n_points << 8, 9, 10, 11, 12, 13, 14, 15, 20, 40, 100, 200;


  std::srand(std::time(nullptr));

  for (size_t noise_id=0; noise_id < n_noise; noise_id++)
  {
    double noise = array_noise(noise_id);

    for (size_t points_id=0; points_id < n_points_id; points_id++)
    {
        size_t n_points = array_n_points(points_id);

        auto name_file = "test_noise" + std::to_string( noise_id ) + "_N_" + std::to_string(points_id) + ".txt";
        ...
        for (int i = 0; i < N_iter; i++)
        {
            ...
            Vector3 translation;
              ...
              EssentialClass my_essential_estimation(points_correspondences);
